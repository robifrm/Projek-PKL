          ...
          <div class="package-card">
            <div class="package-header">
              ...
              <div class="package-speed">
                {{
                  detail.package.speed ? detail.package.speed + " Mbps" : "—"
                }}
              </div>
            </div>
            <div
            ...
            >
              <div class="panel-field">
                ...
                <div class="panel-field-value">
                  {{ formatRp(detail.package.price) }}
                </div>
              </div>
              <div class="panel-field">
                <div class="panel-field-label">PPN 11%</div>
                <div class="panel-field-value">
                  {{ formatRp(Math.round(detail.package.price * 0.11)) }}
                </div>
              </div>
              <div class="panel-field">
                <div class="panel-field-label">Total Bayar</div>
                <div
                  class="panel-field-value text-green"
                  style="font-weight: 700"
                >
                  {{ formatRp(Math.round(detail.package.price * 1.11)) }}
                </div>
              </div>
            </div>
        ...
        <div class="panel-section" v-if="detail.address">
          ...
          <div class="address-block">
            <div class="address-line" v-if="detail.address.alamat">
              ...
              <span v-if="detail.address.rtRw"
                >, RT/RW {{ detail.address.rtRw }}</span
              >
            </div>
            <div class="address-sub">
              <span v-if="detail.address.kelurahan">{{
                detail.address.kelurahan
              }}</span>
              <span v-if="detail.address.kecamatan">
                · {{ detail.address.kecamatan }}</span
              >
            </div>
            <div class="address-city">
              ...
              {{ detail.address.kota || "—" }}
              <span v-if="detail.address.kodePos">
                {{ detail.address.kodePos }}</span
              >
            </div>
          </div>
        </div>
          ...
          <div class="agent-card">
            ...
            <div class="agent-info">
              ...
              <div class="agent-meta">
                ...
                <span class="badge badge--sm" :class="agentStatusClass">{{
                  detail.agent.status || "—"
                }}</span>
              </div>
              <div v-if="detail.agent.noTelpon" class="agent-phone">
                {{ detail.agent.noTelpon }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="panel-section actions-section">
          ...
          <div class="actions-grid">
            <button
              ...
              :class="detail.isolir ? 'btn--activate' : 'btn--isolir'"
      ...
      <div class="confirm-modal-card">
        ...
        <div class="confirm-modal-footer">
          <button
            ...
            @click="showConfirmModal = false"
            style="margin-bottom: 0; width: auto; padding: 8px 16px"
          >
            Batal
          </button>
          <button
...
function copyCustomerInfo() {
  ...
  const addressText = addr
    ? `${addr.alamat || ""}, RT/RW ${addr.rtRw || ""}, ${addr.kelurahan || ""}, ${addr.kecamatan || ""}, ${addr.kota || ""}`
    : "—";
...
Alamat: ${addressText}`;
  ...
  navigator.clipboard
    .writeText(text)
    .then(() =>
      toast.success("Informasi pelanggan berhasil disalin ke clipboard!"),
    )
...
.panel-avatar--isolir {
  background: linear-gradient(135deg, var(--red-warn) 0%, #c0392b 100%);
}
.panel-avatar--default {
...
.package-card .panel-field-label {
  color: rgba(255, 255, 255, 0.6);
}
.package-card .panel-field-value {
  color: #fff;
}
.package-desc {
  margin-top: 10px;
